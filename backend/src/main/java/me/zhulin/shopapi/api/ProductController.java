...
//        }
    ...
    @GetMapping("/product/searchPIPINO")
    public ResponseEntity<List<ProductInfo>> searchProducts(@RequestParam String keyword) {
        List<ProductInfo> products = productService.searchProducts(keyword);
        System.out.println(products.size());
        return new ResponseEntity<>(products, HttpStatus.OK);
    }

    //------
    @GetMapping("/product/searchPIPINOS")
    public Page<ProductInfo> searchProductss(@RequestParam String keyword,
                                             @RequestParam Integer page,
                                             @RequestParam Integer size) {
        List<ProductInfo> products = productService.searchProducts(keyword);
        System.out.println(products.size());
        PageRequest request = PageRequest.of(page - 1, size);
        return productService.searchP(request, keyword);
    }
}
